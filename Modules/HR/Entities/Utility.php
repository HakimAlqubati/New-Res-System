...
class Utility extends Model
{
    ...
    {
        $data = DB::table('hr_settings');

        if (\Auth::check()) {

            $data = $data->where('created_by', '=', \Auth::user()->creatorId())->get();
            if (count($data) == 0) {
                $data = DB::table('hr_settings')->where('created_by', '=', 1)->get();
            }
        } else {
    ...
    //     {
    //         $user = \Auth::user()->id;
    //         $setting = DB::table('hr_settings')->where('created_by',$user)->pluck('value','name')->toArray();

    //     }
    //     else{
    //         $setting = DB::table('hr_settings')->pluck('value','name')->toArray();
    //     }
    //     return $setting;
    ...
    public static function get_superadmin_logo()
    {
        ...
        $is_dark_mode = DB::table('hr_settings')->where('created_by', '1')->pluck('value', 'name')->toArray();
    ...
    public static function get_company_logo()
    {
        ...
        $is_dark_mode = DB::table('hr_settings')->where('created_by', Auth::user()->id)->pluck('value', 'name')->toArray();
    ...
    //         //      $data =\DB::table('hr_settings')->where('created_by', '=', \Auth::user()->id )->get();
    ...
    { 
        if (\Auth::user()) {
            if (\Auth::user()->type == 'super admin') {
                $user = \Auth::user(); 
                $setting = DB::table('hr_settings')->where('created_by', $user->id)->pluck('value', 'name')->toArray();
            } else {
                $setting = DB::table('hr_settings')->where('created_by', \Auth::user()->creatorId())->pluck('value', 'name')->toArray();
            }
        } else {
    ...
    {
        $data = \DB::table('hr_settings')->whereIn('name', ['meta_title', 'meta_description', 'meta_image'])->get();
        $settings = [];
        foreach ($data as $row) {
            $settings[$row->name] = $row->value;
        }
    ...
    {
        $data = \DB::table('hr_settings')->whereIn('name', [
